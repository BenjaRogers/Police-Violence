...
def color_icon(armed):
    if armed == 'unarmed':
        ...
        return 'black'
"""
Offsets markers by random amount of long/lat so that markers do not pile up 
on top of one another. When on top of one another, the lower markers are 
inaccessible 
"""
    ...
    if lt != 'Latitude' and ln != 'Longitude':
        if '2020' in dt:
            ...
            fg20.add_child(folium.CircleMarker(location=[offset_location_lat(lt), offset_location_lon(ln)], popup=folium.Popup(iframe),
                                               radius=5, fill_color=fill_color_selector(rc), fill_opacity=.8,color=color_icon(arm), opacity=1))
        if '2019' in dt:
            iframe = folium.IFrame(html=html % (nm, nm, dt, rc, arm, tl, flee, bc, mod), width=200, height=100)
